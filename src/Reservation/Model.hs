...
data Reservation = Reservation
  { reservationName  :: !Text    -- ^ The name of person that created reservation.
  , reservationMail  :: !Text    -- ^ The e-mail address of reservation issuer.
  , reservationDay   :: !Day     -- ^ The date when reservation will happen.
  , reservationSeats :: !Seats   -- ^ The number of seats to be taken.
...
data ReservationError
  = ReservationAlreadyExists  !Reservation    -- ^ Reservation already exists in store.
  | ReservationNotFound       !Reservation    -- ^ There is no such reservation in store.
  | SeatsNotAvailable         !Seats          -- ^ Cannot add reservation because there is no seat available.
...
addReservation maxSeats r rs
  ...
  where seatsNow = seatsAvailable maxSeats rs
